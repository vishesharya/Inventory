...
function getCurrentNumber($con) {
    $result = mysqli_query($con, "SELECT sheets_issue_temp FROM challan_temp LIMIT 1");
    $row = mysqli_fetch_assoc($result);
    return $row['sheets_issue_temp'];
}

// Function to update the current number in the database
function updateCurrentNumber($con, $newNumber) {
    mysqli_query($con, "UPDATE challan_temp SET sheets_issue_temp = $newNumber");
}
...
if (isset($_POST['add_product'])) {
    // Validate input
    if (empty($_POST['product_name']) || empty($_POST['product_base']) || empty($_POST['product_color']) || empty($_POST['labour_name'])) {
...
if (isset($_POST['submit_products'])) {
    ...
    if (empty($temp_products)) {
        ...
        foreach ($temp_products as $product) {
            ...
            $insert_query = "INSERT INTO sheets_issue (challan_no, labour_name , product_name, product_base, product_color, quantity1, quantity2, quantity3, small_panel_color, date_and_time) 
                            VALUES ('$challan_no','$labour_name', '$product_name', '$product_base', '$product_color', '$quantity1' , '$quantity2', '$quantity3', '$small_sheet_color', '$date_and_time')";
            $insert_result = mysqli_query($con, $insert_query);

             // Insert product into the database
             $insert_query1 = "INSERT INTO sheets_job_work (challan_no_issue, labour_name , product_name, product_base, product_color, quantity1, quantity2, quantity3, small_panel_color, date_and_time) 
             VALUES ('$challan_no','$labour_name', '$product_name', '$product_base', '$product_color', '$quantity1' , '$quantity2', '$quantity3', '$small_sheet_color', '$date_and_time')";
$insert_result1 = mysqli_query($con, $insert_query1);
    ...
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-body">
                        <h1 class="h4 text-center mb-4">Sheets Issue Form</h1>
                        <?php if (!empty($errors)) : ?>
