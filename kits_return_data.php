...
$stitcher_query = "SELECT DISTINCT stitcher_name FROM kits_return ORDER BY stitcher_name ASC"; 
...
// Fetch product names
$product_query = "SELECT DISTINCT product_name FROM kits_return ORDER BY product_name ASC";
...
if ($selected_product) {
    $product_base_query = "SELECT DISTINCT product_base FROM kits_return WHERE product_name = '$selected_product' ORDER BY product_base ASC";
    $product_base_result = mysqli_query($con, $product_base_query);

    // Fetch product colors based on selected product and base
    if ($selected_base) {
        $product_color_query = "SELECT DISTINCT product_color FROM kits_return WHERE product_name = '$selected_product' AND product_base = '$selected_base' ORDER BY product_color ASC";
...
if (isset($_POST['view_entries'])) {
    ...
    $query = "SELECT * FROM kits_return WHERE $conditions";
...
<head>
    ...
    <title>KITS RETURNS DETAILS</title>
...
<body>
    ...
    <div class="container-fluid mt-5">
          <h1 class="h4 text-center mb-4">KITS RETURNS DETAILS </h1> <!-- Changed container to container-fluid -->
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            <table class="table datatable-multi-sorting">
                ...
                while ($data = mysqli_fetch_array($result)) {
                    ...
                    <?php 
                    $sn++;
                    $total_return_quantity += $data['quantity'];
...
<script>
        function fetchChallanNumbers(selectedStitcher, fromDate, toDate) {
            ...
            xhttp.open("GET", "fatch_challan_no_for_kits_return.php?stitcher=" + selectedStitcher + "&from_date=" + fromDate + "&to_date=" + toDate, true);
            xhttp.send(); 
        }
