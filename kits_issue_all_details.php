...
if (isset($_POST['view_entries'])) {
    ...
    if (!empty($stitcher_name)) {
        if (!empty($_POST['from_date']) && !empty($_POST['to_date'])) {
            ...
            $query = "SELECT * FROM kits_issue WHERE stitcher_name = '$stitcher_name' AND date_and_time BETWEEN '$start_date' AND '$end_date'";
        } else {
            // Fetch all entries for the selected stitcher without considering date range
            $query = "SELECT * FROM kits_issue WHERE stitcher_name = '$stitcher_name'";
        }
